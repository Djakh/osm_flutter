...
final class WebOsmController with WebMixin implements IBaseOSMController {
  ...
  WebOsmController() {
    ...
    mapIdMixin = mapId;
    _div = web.document.createElement('div') as web.HTMLDivElement; //web.HTMLDivElement(); //html.DivElement()
    _div.style.width = '100%';
    _div.style.height = '100%';
    ui.platformViewRegistry.registerViewFactory(FlutterOsmPluginWeb.getViewType(mapId), (int viewId) {
  ...
  void createHtml() {
    ...
    if (web.window.document.getElementById("osm_interop") == null) {
      body.appendChild(web.document.createElement('script') as web.HTMLScriptElement
        ..id = "osm_interop"
        ..src = '${kReleaseMode ? "assets/" : ''}packages/flutter_osm_web/src/asset/osm_interop.js'
  ...
  Future<void> initPositionMap({
    ...
    webPlatform.onRoadMapClickListener(mapIdMixin).listen((event) {
      osmWebFlutterState.widget.controller.setValueListenerMapRoadTapping(event.value);
      osmWebFlutterState.widget.controller.osMMixins.forEach((osmMixin) {
    ...
    if (osmWebFlutterState.widget.staticIconGlobalKeys.isNotEmpty) {
      ...
      keys.removeWhere(
          (key, value) => osmWebFlutterState.widget.staticPoints.firstWhere((element) => element.id == key).markerIcon == null);
      keys.forEach((id, key) {
  ...
  Future<void> changeMarker({
    ...
    if (newMarkerIcon != null) {
      duration = 300;
      osmWebFlutterState.widget.dynamicMarkerWidgetNotifier.value = newMarkerIcon;
    }
  ...
  Future customUserLocationMarker(
    ...
    if (directionIconMarkerKey.currentContext != null) {
      final iconPNG = (await capturePng(directionIconMarkerKey)).convertToString();
